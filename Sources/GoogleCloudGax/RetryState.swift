...
///
/// On a request error, the client library queries the retry policy as to whether it should make a
/// new attempt. The client library provides an instance of this type to the retry policy.
///
/// The retry and backoff policies should be stateless, this type is the only information they
/// receive.
///
...
public struct RetryState: Sendable {
  ...
  ///
  /// Some policies retry non-idempotent operations because they are safe for a given configuration
  /// of the service or client.
  ...
  ///
  /// - Parameter idempotent: If true, the request is idempotent and it is safe to retry.
  public init(idempotent: Bool = false) {

...
extension FindingSeverity {
  ...
  internal func toProto() throws -> StorageControlProtos.Google_Storage_Control_V2_FindingSeverity {
    switch self {
    case .unspecified:
      return .unspecified
    case .critical:
    ...
    case .unknownIntValue(let val):
      return StorageControlProtos.Google_Storage_Control_V2_FindingSeverity(rawValue: val)
        ?? .UNRECOGNIZED(val)
    case .unknownStringValue(let str):
      throw GoogleCloudGax.ProtobufConversionError.noIntegerValue(
        enumType: "FindingSeverity", stringValue: str)
    }
  }
}

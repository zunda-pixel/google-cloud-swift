...
func sample(
  ...
  let response = try await client.getPythonPackage(
    request: GetPythonPackageRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/repositories/\(repositoryId)/pythonPackages/\(pythonPackageId)"
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        repositoryId: "[placeholder]", pythonPackageId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }

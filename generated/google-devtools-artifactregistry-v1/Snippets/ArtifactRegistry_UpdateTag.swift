...
func sample(
  ...
  packageId: String, tagId: String
) async throws {
  let response = try await client.updateTag(
    request: UpdateTagRequest()
      .with {
        $0.tag = Tag().with {
          $0.name =
            "projects/\(projectId)/locations/\(locationId)/repositories/\(repositoryId)/packages/\(packageId)/tags/\(tagId)"
        }
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        repositoryId: "[placeholder]", packageId: "[placeholder]", tagId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }

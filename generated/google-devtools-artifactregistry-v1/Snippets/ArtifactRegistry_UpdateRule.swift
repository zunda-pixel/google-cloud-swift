...
func sample(
  ...
  let response = try await client.updateRule(
    request: UpdateRuleRequest()
      .with {
        $0.rule = Rule().with {
          $0.name =
            "projects/\(projectId)/locations/\(locationId)/repositories/\(repositoryId)/rules/\(ruleId)"
        }
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        repositoryId: "[placeholder]", ruleId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }

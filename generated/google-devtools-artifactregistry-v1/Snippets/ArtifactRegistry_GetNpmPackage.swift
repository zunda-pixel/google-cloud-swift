...
func sample(
  ...
  let response = try await client.getNpmPackage(
    request: GetNpmPackageRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/repositories/\(repositoryId)/npmPackages/\(npmPackageId)"
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        repositoryId: "[placeholder]", npmPackageId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }

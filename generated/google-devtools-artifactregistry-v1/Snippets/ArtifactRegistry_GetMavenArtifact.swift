...
func sample(
  ...
  let response = try await client.getMavenArtifact(
    request: GetMavenArtifactRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/repositories/\(repositoryId)/mavenArtifacts/\(mavenArtifactId)"
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        repositoryId: "[placeholder]", mavenArtifactId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }

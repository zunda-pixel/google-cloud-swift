...
func sample(
  ...
  caPoolId: String, certificateId: String
) async throws {
  let response = try await client.updateCertificate(
    request: UpdateCertificateRequest()
      .with {
        $0.certificate = Certificate().with {
          $0.name =
            "projects/\(projectId)/locations/\(locationId)/caPools/\(caPoolId)/certificates/\(certificateId)"
        }
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        caPoolId: "[placeholder]", certificateId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }

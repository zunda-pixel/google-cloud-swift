...
func sample(
  ...
  let poller = try await client.updateCertificateRevocationList(
    withPolling: UpdateCertificateRevocationListRequest()
      .with {
        $0.certificateRevocationList = CertificateRevocationList().with {
          $0.name =
            "projects/\(projectId)/locations/\(locationId)/caPools/\(caPoolId)/certificateAuthorities/\(certificateAuthorityId)/certificateRevocationLists/\(certificateRevocationListId)"
        }
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        caPoolId: "[placeholder]", certificateAuthorityId: "[placeholder]",
        certificateRevocationListId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }

  ...
  let response = try await client.updateTag(
    request: UpdateTagRequest()
      .with {
        $0.tag = Tag().with {
          $0.name =
            "projects/\(projectId)/locations/\(locationId)/entryGroups/\(entryGroupId)/entries/\(entryId)/tags/\(tagId)"
        }
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        entryGroupId: "[placeholder]", entryId: "[placeholder]", tagId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }

...
// limitations under the License.
  ...
  func sample(
    ...
    evaluationId: String, sliceId: String
  ) async throws {
    let response = try await client.getModelEvaluationSlice(
      request: GetModelEvaluationSliceRequest()
        .with {
          $0.name =
            "projects/\(projectId)/locations/\(locationId)/models/\(modelId)/evaluations/\(evaluationId)/slices/\(sliceId)"
        }
    )
    ...
    static func main() async throws {
      do {
        ...
        try await sample(
          ...
          modelId: "[placeholder]", evaluationId: "[placeholder]", sliceId: "[placeholder]")
      } catch {
        print("Error: \(error)")
      }
    }

...
// limitations under the License.
  ...
  func sample(
    ...
    let response = try await client.getMetadataSchema(
      request: GetMetadataSchemaRequest()
        .with {
          $0.name =
            "projects/\(projectId)/locations/\(locationId)/metadataStores/\(metadataStoreId)/metadataSchemas/\(metadataSchemaId)"
        }
    )
    ...
    static func main() async throws {
      do {
        ...
        try await sample(
          ...
          metadataStoreId: "[placeholder]", metadataSchemaId: "[placeholder]")
      } catch {
        print("Error: \(error)")
      }
    }

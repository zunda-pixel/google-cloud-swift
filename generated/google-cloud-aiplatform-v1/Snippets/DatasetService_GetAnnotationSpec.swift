...
// limitations under the License.
  ...
  func sample(
    ...
    let response = try await client.getAnnotationSpec(
      request: GetAnnotationSpecRequest()
        .with {
          $0.name =
            "projects/\(projectId)/locations/\(locationId)/datasets/\(datasetId)/annotationSpecs/\(annotationSpecId)"
        }
    )
    ...
    static func main() async throws {
      do {
        ...
        try await sample(
          ...
          datasetId: "[placeholder]", annotationSpecId: "[placeholder]")
      } catch {
        print("Error: \(error)")
      }
    }

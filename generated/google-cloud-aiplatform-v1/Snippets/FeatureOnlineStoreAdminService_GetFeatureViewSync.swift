...
// limitations under the License.
  ...
  func sample(
    ...
    let response = try await client.getFeatureViewSync(
      request: GetFeatureViewSyncRequest()
        .with {
          $0.name =
            "projects/\(projectId)/locations/\(locationId)/featureOnlineStores/\(featureOnlineStoreId)/featureViews/\(featureViewId)/featureViewSyncs/feature_view_sync"
        }
    )
    ...
    static func main() async throws {
      do {
        ...
        try await sample(
          ...
          featureOnlineStoreId: "[placeholder]", featureViewId: "[placeholder]")
      } catch {
        print("Error: \(error)")
      }
    }

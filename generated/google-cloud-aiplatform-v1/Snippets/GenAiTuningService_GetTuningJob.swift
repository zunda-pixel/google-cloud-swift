...
// limitations under the License.
  ...
  func sample(
    client: GenAiTuningServiceClient, projectId: String, locationId: String, tuningJobId: String
  ) async throws {
    let response = try await client.getTuningJob(
      request: GetTuningJobRequest()
        .with {
          $0.name = "projects/\(projectId)/locations/\(locationId)/tuningJobs/\(tuningJobId)"
        }
    )

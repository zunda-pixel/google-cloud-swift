...
// limitations under the License.
  ...
  func sample(
    ...
    let response = try await client.updateTensorboardTimeSeries(
      request: UpdateTensorboardTimeSeriesRequest()
        .with {
          $0.tensorboardTimeSeries = TensorboardTimeSeries().with {
            $0.name =
    ...
    static func main() async throws {
      do {
        ...
        try await sample(
          ...
          tensorboardId: "[placeholder]", experimentId: "[placeholder]", runId: "[placeholder]",
          timeSeriesId: "[placeholder]")
      } catch {
        print("Error: \(error)")
      }
    }

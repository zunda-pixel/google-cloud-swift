...
func sample(
  ...
  let response = try await client.getConnectorVersion(
    request: GetConnectorVersionRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/providers/\(providerId)/connectors/\(connectorId)/versions/\(versionId)"
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        providerId: "[placeholder]", connectorId: "[placeholder]", versionId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }

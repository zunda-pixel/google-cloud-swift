...
func sample(
  ...
  let response = try await client.getAutomationRun(
    request: GetAutomationRunRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/deliveryPipelines/\(deliveryPipelineId)/automationRuns/\(automationRunId)"
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        deliveryPipelineId: "[placeholder]", automationRunId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }

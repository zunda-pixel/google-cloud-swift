...
func sample(
  ...
  let response = try await client.getPartition(
    request: GetPartitionRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/lakes/\(lakeId)/zones/\(zoneId)/entities/\(entityId)/partitions/\(partitionId)"
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        lakeId: "[placeholder]", zoneId: "[placeholder]", entityId: "[placeholder]",
        partitionId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }

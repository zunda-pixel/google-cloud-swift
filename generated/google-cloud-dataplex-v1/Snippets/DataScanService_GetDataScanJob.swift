...
func sample(
  ...
  let response = try await client.getDataScanJob(
    request: GetDataScanJobRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/dataScans/\(dataScanId)/jobs/\(jobId)"
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        dataScanId: "[placeholder]", jobId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }

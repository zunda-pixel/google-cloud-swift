...
func sample(
  ...
  let response = try await client.updateListing(
    request: UpdateListingRequest()
      .with {
        $0.listing = Listing().with {
          $0.name =
            "projects/\(projectId)/locations/\(locationId)/dataExchanges/\(dataExchangeId)/listings/\(listingId)"
        }
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        dataExchangeId: "[placeholder]", listingId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }

...
func sample(
  ...
  let response = try await client.getImportJob(
    request: GetImportJobRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/keyRings/\(keyRingId)/importJobs/\(importJobId)"
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        keyRingId: "[placeholder]", importJobId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }

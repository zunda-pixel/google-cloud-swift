...
// snippet.show
import Foundation
import GoogleCloudBeyondcorpAppconnectionsV1
...
func sample(
  ...
  let poller = try await client.updateAppConnection(
    withPolling: UpdateAppConnectionRequest()
      .with {
        $0.appConnection = AppConnection().with {
          $0.name =
            "projects/\(projectId)/locations/\(locationId)/appConnections/\(appConnectionId)"
        }

...
func sample(
  ...
  let response = try await client.getAuthorizedView(
    request: GetAuthorizedViewRequest()
      .with {
        $0.name =
          "projects/\(projectId)/instances/\(instanceId)/tables/\(tableId)/authorizedViews/\(authorizedViewId)"
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        tableId: "[placeholder]", authorizedViewId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
